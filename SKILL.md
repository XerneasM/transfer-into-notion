...
10. Create a knowledge visualization in efficient standard/deep mode. Read [references/knowledge-map-design.md](references/knowledge-map-design.md) before art direction. Default to `hybrid`: spend the user's currently available built-in AI-image allowance on a text-free aesthetic base, then overlay exact labels deterministically. Never silently use an external paid API or silently downgrade. Persist `local_svg` only after the user explicitly says allowance consumption is too high, asks to save allowance, or requests local-only output. Invent a real reference style whose family, layout, palette, typography, motifs, and information organization are materially different from compact style history. A style name is not sufficient: its visible grammar must be materialized by actual scene objects. Run the local SVG v2 `schema`, `validate`, `audit`, and `render` workflow, then inspect a preview at full and thumbnail size. A generic card-and-arrow office flowchart is a blocking failure even when technical validation passes.
