...
10. Create a knowledge visualization in efficient standard/deep mode. Read [references/knowledge-map-design.md](references/knowledge-map-design.md) before art direction. If local state has no `visualization_mode`, ask the user once whether future maps should default to local advanced SVG, hybrid SVG + image generation, or asking every time; persist the choice with `state_manager.py set-visualization-mode`. Invent a real reference style whose family, layout, palette, typography, motifs, and information organization are materially different from compact style history. A style name is not sufficient: its visible grammar must be materialized by actual scene objects. Run the local SVG v2 `schema`, `validate`, `audit`, and `render` workflow, then inspect a preview at full and thumbnail size. A generic card-and-arrow office flowchart is a blocking failure even when technical validation passes. Hybrid image generation is optional and never required for local portability.
