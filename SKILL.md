...
10. Create a knowledge visualization in efficient standard/deep mode. Read [references/knowledge-map-design.md](references/knowledge-map-design.md) before art direction. Default to `hybrid`: spend the user's currently available built-in AI-image allowance on one integrated knowledge map in which AI designs the information structure, hierarchy, composition, typography, connectors, visual metaphor, and overall aesthetic together. Do not restrict AI to a small icon, partial texture, or background layer, and do not replace its information architecture with deterministic local SVG by default. Freeze an exact-content brief before generation, inspect the result at full and thumbnail size, and prefer AI editing or regeneration when structure, labels, or readability fail; local editing is limited to surgical exact-text repair after the integrated composition is accepted. Never silently use an external paid API or silently downgrade. Persist `local_svg` only after the user explicitly says allowance consumption is too high, asks to save allowance, or requests local-only output; only that mode runs the local SVG v2 workflow as the primary renderer. Use compact style history and `scripts/select_knowledge_map_style.py` to choose stably among source-appropriate candidates. Retro and historical styles remain allowed, but two clearly retro/historical styles must not appear consecutively, and adjacent styles must differ materially in family and visible grammar. Both a generic card-and-arrow office flowchart and decorative overload are blocking failures even when the image is technically valid.
